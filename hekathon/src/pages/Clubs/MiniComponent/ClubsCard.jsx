...
const ClubsCard = ({ club }) => {
  ...
  return (
    <div onClick={()=>navigation(`clubs/${club.id}`)} className="max-w-lg md:m-2 my-2 p-2 rounded overflow-hidden shadow-lg">
      <img className="w-full h-72 rounded object-center" src={club?.image} alt={club.name} />

        <div className=" py-4 px-6 text-primary cursor-pointer  font-bold text-xl mb-2 ">{club?.name}</div>
